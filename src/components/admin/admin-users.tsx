    ...
    <div className="space-y-3">
      ...
      {cargando ? (
        ...
        <ul className="space-y-2">
          {usuarios.map((user) => (
            <li key={user.profileId} className="rounded-2xl bg-white p-3 text-ink">
              <div className="flex flex-wrap items-start justify-between gap-2">
                <div className="min-w-0">
                  <p className="flex items-center gap-1.5 truncate font-display text-title-card">
                    {user.name}
                    {user.role === 'admin' && <ShieldCheck size={14} className="shrink-0 text-party-primary" />}
                  </p>
                ...
                <span
                  className={cn(
                    'shrink-0 rounded-full px-2 py-0.5 text-caption font-bold',
                    user.status === 'active' ? 'bg-black/[0.06] text-ink/60' : 'bg-destructive/15 text-destructive',
                  )}
                >
                  {user.status}
                </span>
              ...
              <div className="mt-3 flex flex-wrap items-center gap-2">
                ...
                <span className="ml-auto flex items-center gap-1 rounded-full bg-[#F5F5F7] px-1 py-1">
                  <button
                    type="button"
