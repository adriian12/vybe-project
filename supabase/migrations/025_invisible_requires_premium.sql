-- ============================================================================
-- 025. El modo invisible sólo cuenta si hay suscripción activa
-- ============================================================================
-- El modo invisible deja ver a todo el mundo sin que nadie te vea. Es una
-- ventaja unilateral sobre el resto del tablón, así que es de pago.
--
-- Hasta ahora la comprobación vivía sólo en la interfaz, y la interfaz se puede
-- saltar: bastaba una llamada a `update_profile` con `is_invisible = true` para
-- quedarse invisible sin pagar, y para seguir invisible cuando la suscripción
-- caducara.
--
-- La regla pasa aquí, que es donde se decide quién sale en el tablón. La
-- columna se puede seguir marcando —guarda la preferencia—, pero sólo surte
-- efecto mientras `is_premium()` diga que sí.
--
-- Es la misma función de la migración 018, con una única cláusula cambiada:
...
COMMENT ON FUNCTION public.get_nearby_profiles IS
    ...
    'Tablón del evento. Exige foto de esta noche en las dos direcciones, y el modo invisible sólo surte efecto con suscripción activa.';
