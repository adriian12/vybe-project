 ...
 *
 * El diseño es el común de los correos de Fiestea (`_shared/email.ts`).
 */
...
serve(async (req: Request): Promise<Response> => {
  ...
  try {
    ...
    const resetUrl = `${BRAND.app}/auth/reset-password`;
    const redirectTo = linkType === 'recovery' ? resetUrl : `${BRAND.app}/auth/verify-email`;
    ...
    const send = async (kind: Kind, link: string): Promise<Response | null> => {
      ...
      const { html, text } = renderEmail({
        ...
        buttons: [{ text: copy.cta, url: link }],
        showLink: link,
        note: copy.ignore,
      });
      try {
